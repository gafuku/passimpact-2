...
import { institutions, getLatestReport } from "../../components/report/reportData";
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {tracked.map((institution) => {
              const latest = getLatestReport(institution.slug)!;
              return (
                <div key={institution.slug} className="border border-border bg-white p-5 flex flex-col">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-text-muted mt-0.5">{institution.type} · {institution.location}</p>
                    </div>
                    <button
                  ...
                  <div className="mt-4 flex items-center gap-4 text-xs text-text-muted">
                    <span>{latest.fy} revenue: <span className="text-text font-medium">${latest.totalRevenue.toFixed(2)}B</span></span>
                    <span>Endowment: <span className="text-text font-medium">${latest.endowment.toFixed(1)}B</span></span>
                  </div>
