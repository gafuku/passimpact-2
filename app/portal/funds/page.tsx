...
export default function PortalFundsPage() {
  ...
  const { namedFunds, loading: fundsLoading, error: fundsError } = usePublicNamedFunds();
  const { institutions, loading: institutionsLoading } = usePublicInstitutions();

  const loading = fundsLoading || institutionsLoading;
  const getInstitution = (slug: string) => institutions.find((i) => i.slug === slug);
        ...
        <>
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="text-xs text-text-muted mt-0.5">
                            {institution?.name ?? "Unknown institution"} · {fund.restrictionType} · established {fund.established}
                          </p>
                        </div>
                        <button
          ...
          <div className="mt-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                return (
                  <div key={fund.id} className="border border-border bg-white p-5 flex items-start justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold text-text">{fund.name}</p>
                      <p className="text-xs text-text-muted mt-0.5">
                        {institution?.name ?? "Unknown institution"} · {fund.restrictionType}
                      </p>
                    </div>
                    <button
                      onClick={() => toggleFollowFund(fund.id)}
              ...
              {others.length === 0 && namedFunds.length > 0 && (
                <p className="col-span-full text-xs text-text-faint italic py-4">You're tracking every named fund we have on file.</p>
              )}
              {namedFunds.length === 0 && (
                <p className="col-span-full text-xs text-text-faint italic py-4">
                  No named funds have been published yet.
                </p>
              )}
            </div>
          </div>
